...
async function main() {
  ...
  const paypalPhoneLines = fs.readFileSync('/root/payalphone', 'utf8').trim().split('\n');
  ...
  // hostedCheckoutSmsPoolText: 多行文本，格式 "电话----URL"
  const hostedCheckoutSmsPoolText = paypalPhoneLines.join('\n');
  // hostedCheckoutPhoneNumber: 第一个电话号码
  const firstPhone = paypalPhoneLines[0]?.split('----')[0] || '';
  ...
  const config = {
    ...
    hostedCheckoutPhoneNumber: firstPhone,
    ...
    expression: `new Promise((resolve) => {
      ...
      chrome.storage.local.set(config, () => {
        chrome.storage.local.get([
          'hotmailAccounts', 'hostedCheckoutSmsPoolText', 'hostedCheckoutPhoneNumber',
          'plusCheckoutCloudConversionEnabled', 'panelMode'
        ], (result) => {
          resolve({
            ok: true,
            panelMode: result.panelMode,
            hotmailCount: result.hotmailAccounts?.length || 0,
            smsPoolText: result.hostedCheckoutSmsPoolText?.substring(0, 60) || 'empty',
            phone: result.hostedCheckoutPhoneNumber || 'empty',
