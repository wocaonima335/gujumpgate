...
(async () => {
  try {
    ...
    let spTarget = list.find(t => t.url.includes('sidepanel/sidepanel.html'));
    ...
    if (!spTarget) {
      ...
      spTarget = newList.find(t => t.url.includes('sidepanel/sidepanel.html'));
    }
    ...
    const rv = await Runtime.evaluate({
      expression: `(async () => {
        const d = await chrome.storage.local.get(['hotmailAccounts','mailProvider','plusModeEnabled','plusPaymentMethod','hostedCheckoutPhone','hostedSmsPoolEntries','phoneVerificationEnabled']);
