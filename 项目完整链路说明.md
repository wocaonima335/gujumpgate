...
面向外部 Agent / CDP runner 的推荐控制方式，当前不再建议继续直接依赖 sidepanel DOM、旧的 `START_AUTO_RUN` 消息或零散的 `check-* / click-*` 探针脚本拼装流程状态。

组成：

- [agent-control.html](e:/GuJumpgate/GuJumpgate/agent-control.html)
- [agent-control.js](e:/GuJumpgate/GuJumpgate/agent-control.js)
- [cdp-controller.js](e:/GuJumpgate/GuJumpgate/cdp-controller.js)
- [run-headless.js](e:/GuJumpgate/GuJumpgate/run-headless.js)

链路：

1. 外部 runner 通过 CDP 打开 `chrome-extension://<extension-id>/agent-control.html`
2. `agent-control.js` 在扩展页上下文里暴露 `window.GuJumpgateAgentControl.*`
3. 外部 runner 只调用统一方法，例如 `applyProfile / startRun / stopRun / getSnapshot / captureDiagnostics`
4. `agent-control.js` 再向后台发送既有 runtime message，例如 `SAVE_SETTING / AUTO_RUN / STOP_FLOW / GET_STATE`
5. 后台继续复用原有步骤引擎、自动运行控制器、内容脚本和 provider 逻辑，不在 runner 里重复实现业务流程
6. 状态读取优先走 `getSnapshot()` 返回的低噪音结构化摘要；需要排障时再调用 `captureDiagnostics()`
7. `run-headless.js` 与 `cdp-controller.js` 都应优先复用这条控制链，不再直接向 sidepanel 注入脚本或手写 service worker 消息

约定：

- `agent-control` 作为独立控制面，只负责机器调用，不承载人工配置 UI
- 若需要锁定自动化窗口，`agent-control` 与 `sidepanel` 享受同一套后台窗口锁定逻辑
- 外部控制器应优先调用 `AUTO_RUN`，不要再继续发送旧的 `START_AUTO_RUN`
...
- 当前 `STOP_FLOW` 还支持通过 `payload.stopReason` 区分中断来源：用户主动停止继续沿用原有 stop 语义，而后台为了自动重开/恢复主动中断内容脚本时，会标记为内部恢复中断，避免误写成“已被用户停止”
...
5. 执行 `runAutoSequenceFromStep`
   ...
   - 当前若节点长时间没有新日志，watchdog 仍会中断当前内容脚本并准备自动重开；但这类内部中断不再伪装成“用户停止”
6. 如果失败，根据设置决定：
   - 立即停止
   - 当前轮重试
   - 下一轮继续
...
10. 除了 `scheduled / waiting_interval` 计时计划恢复外，如果 Service Worker 在 `running / waiting_step / waiting_email / retrying` 阶段被回收，后台重启后也会尝试基于 `chrome.storage.session` 中保存的 `autoRunCurrentRun / autoRunAttemptRun / autoRunSessionId / nodeStatuses` 恢复自动流程，并把遗留的 running 节点改回 pending 后继续
