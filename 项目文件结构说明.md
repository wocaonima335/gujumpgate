...
- `agent.md`：面向执行任务的大模型 / 外部 Agent 的仓库级执行规范，要求优先通过 `agent-control` 链路控制项目，而不是继续直接依赖 sidepanel DOM、旧消息协议或零散探针脚本。
...
- `agent-control.html`：供外部 Agent / CDP runner 打开的扩展控制页，不承载 sidepanel UI，而是暴露统一的后台控制入口与低噪音状态快照。
- `agent-control.js`：Agent 控制页脚本，负责把 `SAVE_SETTING / AUTO_RUN / STOP_FLOW / GET_STATE` 等后台消息封装为稳定的 `window.GuJumpgateAgentControl.*` API，并缓存近期结构化事件。
- `cdp-controller.js`：外部 CDP 控制器，当前已改为优先连接 `agent-control.html`，通过统一 API 注入配置、启动自动运行、停止流程、抓取诊断与截图，避免继续直接依赖 sidepanel DOM 或旧消息名。
- `run-headless.js`：无头浏览器 runner，当前已改为“启动 Chrome -> 打开 `agent-control.html` -> 注入 profile -> 启动自动运行 -> 轮询结构化快照”的链路；成功优先判断新增认证文件，失败会落地诊断 JSON 与截图。
...
- `background/logging-status.js`：后台日志、步骤状态、错误信息和若干状态判断的公共工具层；日志条目统一写入结构化 `step / stepKey`，sidepanel 只读取该元数据渲染步骤标签，不再从日志正文解析步骤号；当前额外承接 `add-phone / 手机号页` 这类认证 fatal 错误的共享判定，并会把 Step 2 的“手机号输入模式未切成功”与真正的 auth `add-phone` 页面区分开，避免自动运行误停机；停止相关错误会继续沿用统一状态流，但日志层现在会补充区分中断来源。
...
- `background/steps/create-plus-checkout.js`：Plus 模式第 6 步实现，负责在已登录 ChatGPT 页面创建 PayPal / GoPay checkout session，或在 GPC 模式下读取 accessToken 并通过 `https://your-gpc-helper-domain.example/api/gp/tasks` 创建队列任务，记录 checkout / GPC task 运行态；当前 hosted checkout 异步链路会把“后台内部恢复/重试触发的页面中断”与“用户主动停止”区分开，避免误把内部重开记成用户 Stop。
...
- `content/utils.js`：内容脚本公共工具层，负责结构化日志、READY / COMPLETE / ERROR 上报、元素等待、输入与点击；内容脚本日志通过 `{ step, stepKey }` 上报，正文不再承担步骤号解析职责；当前 `STOP_FLOW` 已支持通过 `payload.stopReason` 区分“用户停止”和“后台内部恢复中断”，从而减少 hosted checkout / 自动重开场景下的误报。
...
- `docs/Agent后台控制使用指导.md`：面向外部 Agent / CDP runner / 无头浏览器控制器的使用手册，说明推荐控制链路、profile 格式、`cdp-controller.js` 与 `run-headless.js` 的用法、状态快照和诊断策略。
